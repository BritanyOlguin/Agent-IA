...
def generar_ejemplos_entrenamiento():
    ...
    if not registro:
        print("No hay datos de feedback para generar ejemplos.")
        return
    ...
    if not entradas_incorrectas:
        print("No hay ejemplos incorrectos para generar entrenamiento.")
        return
    
    # Formato para los ejemplos
    ejemplos = []
    for entrada in entradas_incorrectas:
        sugerencia = entrada.get('sugerencia')
        if sugerencia and isinstance(sugerencia, dict):
            ejemplo = {
    ...
    ruta_ejemplos = os.path.join(RUTA_FEEDBACK, "ejemplos_entrenamiento.json")
    with open(ruta_ejemplos, 'w', encoding='utf-8') as f:
        json.dump(ejemplos, f, ensure_ascii=False, indent=2)
    
    print(f"✓ Generados {len(ejemplos)} ejemplos para entrenamiento en {ruta_ejemplos}")
    return ruta_ejemplos
...
def generar_ejemplos_entrenamiento():
    ...
    # Filtrar solo entradas con feedback negativo (incorrectas)
    entradas_incorrectas = [r for r in registro if not r['es_correcto']]
    
    if not entradas_incorrectas:
        print("No hay ejemplos incorrectos para generar entrenamiento.")
        return
    
    # Formato para los ejemplos (similar al de generar_datos.py)
    ejemplos = []
    for entrada in entradas_incorrectas:
...
def interpretar_pregunta_llm(prompt: str, llm_clasificador) -> dict:
    ...
    Ahora incorpora aprendizaje de retroalimentación previa.
    """

    registro = cargar_registro_feedback()

    if registro:
        # Encontrar consultas similares con feedback negativo y sugerencias
        ejemplos_relevantes = []
        for entrada in registro:
            if entrada['prompt'] and prompt:
                similitud_consulta = similitud(normalizar_texto(entrada['prompt']), normalizar_texto(prompt))
                if similitud_consulta > 0.7 and entrada.get('sugerencia'):  # Umbral de similitud
                    ejemplos_relevantes.append((similitud_consulta, entrada))
        
        # Ordenar por similitud y tomar el más relevante
        if ejemplos_relevantes:
            ejemplos_relevantes.sort(reverse=True, key=lambda x: x[0])
            mejor_ejemplo = ejemplos_relevantes[0][1]
            
            # Usar la sugerencia como base para el análisis
            if mejor_ejemplo.get('sugerencia'):
                print(f"[INFO] Usando aprendizaje de feedback previo (similitud: {ejemplos_relevantes[0][0]:.2f})")
                return {
                    "tipo_busqueda": mejor_ejemplo['sugerencia'].get('tipo_busqueda'),
                    "campo": mejor_ejemplo['sugerencia'].get('campo'),
                    "valor": extraer_valor(prompt)  # Extraer el valor del prompt actual
                }
...
def desambiguar_consulta(analisis: dict, prompt: str, llm) -> dict:
    ...
    if registro:
        ...
        if ejemplos_relevantes:
            ...
            # Usar la sugerencia como base para la desambiguación
            if mejor_ejemplo.get('sugerencia'):
                print(f"[INFO] Usando aprendizaje de feedback previo para desambiguar (similitud: {ejemplos_relevantes[0][0]:.2f})")
                return {
                    "tipo_busqueda": mejor_ejemplo['sugerencia'].get('tipo_busqueda'),
                    "campo": mejor_ejemplo['sugerencia'].get('campo'),
                    "valor": extraer_valor(prompt)  # Extraer el valor del prompt actual
...
print("    Además, puedes proporcionar retroalimentación para que el agente aprenda.")

while True:
    prompt = input("\nPregunta: ")
    if prompt.lower() == 'salir':
        break
    if prompt.lower() == 'generar_ejemplos':
